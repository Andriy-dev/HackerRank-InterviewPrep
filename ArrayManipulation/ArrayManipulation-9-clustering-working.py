    ...
    class ctree:
        ...
        def insert(self,vector):
            ...
            if self.cluster is None: 
                # Not exist yet
                # Creating cluster of size cluster_size(=c)
                # Size of each segment would be self.size/cluster_size(=c) 

                self.cluster=[]
                cluster_size=int(self.size/c)

                #if __debug__:
                #    print("Creating cluster size {} with {} elements under self.size={}".format(cluster_size,c,self.size))

                for i in range(c):
                    self.cluster.append(ctree(self.start+cluster_size*i,cluster_size))

            # inserting Into existing cluster
